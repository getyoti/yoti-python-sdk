...
class RequestedThirdPartyCheck(RequestedCheck):
    ...
    Check is perform on user, not document.
    At least one document is needed for extracting data used in check.
    If not documents are provided the Supplementary Document is optional.
    """
...
class RequestedThirdPartyCheckBuilder(object):
    ...
    def build(self):
        return RequestedThirdPartyCheck(
            config=RequestedThirdPartyCheckConfig(manual_check=self.__manual_check)
        )